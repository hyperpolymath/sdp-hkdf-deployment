...
# setup.sh — Universal setup script for sdp-hkdf-deployment
#
# Detects your shell, platform, and installs prerequisites.
# Then hands off to `just setup` for project-specific configuration.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/hyperpolymath/sdp-hkdf-deployment/main/setup.sh | sh
#   # or after cloning:
#   ./setup.sh
#
...
# ── Main ──
main() {
    printf "%s=== sdp-hkdf-deployment Setup ===%s\n\n" "$BOLD" "$RESET"
    ...
    if [ ! -f "Justfile" ] && [ ! -f "justfile" ]; then
        ...
        info "Clone first: git clone https://github.com/hyperpolymath/sdp-hkdf-deployment.git"
        info "Then: cd sdp-hkdf-deployment && ./setup.sh"
        exit 1
    fi
